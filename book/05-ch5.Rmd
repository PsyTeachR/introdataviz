...
-   `size` lets you change the size of lines and points. If you want different groups to be different sizes, set this inside the `aes()` function; if you want to change the size for all groups, set it inside the relevant `geom_*()` function'.
...
The width, height and resolution of the image can all be manually adjusted. Fonts will scale with these sizes, an may look different to the preview images you see in the Viewer tab. The help documentation for is useful here (type `?ggsave` in the console to access the help).
